...
public class LoginPage {
	...
	@FindBy(xpath = "//*[text()='What Next?']")
	private WebElement whatNextText;
	@FindBy(xpath = "//div[@class='boxesRow group']/div/article")
	private List<WebElement> whatNextContent;
	@FindBy (xpath = "//span[text()='Trending Videos']")
	private WebElement trendingVideosText;
	@FindBy (xpath= "//div[contains(@class,' slick-active')]/div")
	private List<WebElement> trendingVideosContent;
	...
	public void signInBtnClick(WebDriver driver) {
		...
		// This code needs to uncommented when any webengange notification starts
		// coming.
	...
	public void checkOnlyHeader(WebDriver driver) {
		...
		/*
		 * coursemenu.click(); BasePage.sleepForMilliSecond(1000);
		...
		/*
		 * anamenu.click(); BasePage.sleepForMilliSecond(1000);
		...
		/*
		 * memenu.click(); BasePage.sleepForMilliSecond(1000);
	...
	public void checkHeaderDisplay(WebDriver driver) {
		...
		// Assert.assertTrue(BasePage.isPresentAndDisplayed(rightbannercontent.get(0)),"Right
		// side banner content not displayed");
		Assert.assertTrue(BasePage.isPresentAndDisplayed(rightbannercontent.get(1)),
				"Right side banner content not displayed");
	...
	public void checkHeaderDisplayForAllClassesSpecificUsers(WebDriver driver, int k) {
		 ...
		 */
		while (k == 13) {
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePaper_NeetCatalystUser),
					"Sample paper for Neet Catalyst user doesn't displayed");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(prvsYrPaper_NeetCatalystUser),
					"Previous Year paper for Neet Catalyst user doesn't displayed");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSession_NeetCatalystUser),
					"Test Sessions for Neet Catalyst user doesn't displayed");
			WaitStatementLib.explicitWaitForVisiblity(driver, 5, testSession_NeetCatalystUser);
			testSession_NeetCatalystUser.click();
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSessionValueOne_NeetCatalystUser),
					"Value AIPMT is not dislplayed for Neet Catalyst User");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSessionValueTwo_NeetCatalystUser),
					"Value AIIMS is not dislplayed for Neet Catalyst User");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSessionValueThree_NeetCatalystUser),
					"Value NEET	 is not dislplayed for Neet Catalyst User");
			...
			Assert.assertTrue(itemsOnTestSessions_NeetCatalystUser.size() == 3,
					"All items are not displayed on Test Session window");
			...
			Assert.assertTrue(BasePage.isPresentAndDisplayed(preNotesOnTestSession_NeetCatalystUser),
					"Prenotes on test session not displayed");
			Assert.assertTrue(preNotesOnTestSession_NeetCatalystUser.getText().length() > 10,
					"Contents of Prenotes on test session not displayed");
			BasePage.sleepForMilliSecond(1000);
			...
			Assert.assertTrue(BasePage.isPresentAndDisplayed(viewSyllabusOnTestSession_NeetCatalystUser),
					"view syllabus link not displayed");
			logo.click();
			testSession_NeetCatalystUser.click();
			samplePapermain_NeetCatalystUser.click();
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePapermainvalueone_NeetCatalystUser),
					"Value AIPMT not displayed on sample paper");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePapermainvalueTwo_NeetCatalystUser),
					"Value AIIMS not displayed on sample paper");
			...
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePapermainvalueone_NeetCatalystUser),
					"Value AIPMT not displayed on Previous Year question paper");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePapermainvalueTwo_NeetCatalystUser),
					"Value AIIMS not displayed on Previous Year question paper");
			WaitStatementLib.explicitWaitForVisiblity(driver, 10, samplePprLst_NeetCatalystUser.get(0));
			Assert.assertTrue(samplePprLst_NeetCatalystUser.size() > 5, "All Sample papers not displayed");
			allBtnssamplePprLst_NeetCatalystUser.get(0).click();
			BasePage.sleepForMilliSecond(1000);
		...
		while (k == 14) {
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePaper_NeetCatalystUser),
					"Sample paper for IIT Power Prep User doesn't displayed");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(prvsYrPaper_NeetCatalystUser),
					"Previous Year paper for IIT Power Prep User doesn't displayed");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSession_NeetCatalystUser),
					"Test Sessions for IIT Power Prep User doesn't displayed");
			WaitStatementLib.explicitWaitForVisiblity(driver, 5, testSession_NeetCatalystUser);
			testSession_NeetCatalystUser.click();
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSessionValueOne_IITPowerPrepUser),
					"Value JEE-Advance is not dislplayed for IIT Power Prep User");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(testSessionValueTwo_IITPowerPrepUser),
					"Value JEE-Mains is not dislplayed for IIT Power Prep User");
			Assert.assertFalse(totalTestSessions_NeetCatalystUser.isEmpty(), "No Test Sessions displayed");
			WaitStatementLib.explicitWaitForVisiblity(driver, 5, totalTestSessions_NeetCatalystUser.get(0));
			viewDetailsBtnOnTestSessions_NeetCatalystUser.get(0).click();
			...
			testSession_NeetCatalystUser.click();
			samplePapermain_NeetCatalystUser.click();
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePaperMainValueOne_IITPowerPrepUser),
					"Value JEE-Advance not displayed on sample paper");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(samplePaperMainValueTwo_IITPowerPrepUser),
					"Value JEE-Mains not displayed on sample paper");
			WaitStatementLib.explicitWaitForVisiblity(driver, 10, samplePprLst_NeetCatalystUser.get(0));
			Assert.assertTrue(samplePprLst_NeetCatalystUser.size() > 5, "All Sample papers not displayed");
			allBtnssamplePprLst_NeetCatalystUser.get(0).click();
			BasePage.sleepForMilliSecond(1000);
			...
			Assert.assertTrue(BasePage.isPresentAndDisplayed(previousYrQuestion_NeetCatalystUser));
			previousYrQuestion_NeetCatalystUser.click();
			Assert.assertTrue(BasePage.isPresentAndDisplayed(prvsYrQValueOne_IITPowerPrepUser),
					"Value AIEEE not displayed on Previous Year question paper");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(prvsYrQValueTwo_IITPowerPrepUser),
					"Value JEE-Advance not displayed on Previous Year question paper");
			Assert.assertTrue(BasePage.isPresentAndDisplayed(prvsYrQValueThree_IITPowerPrepUser),
					"Value JEE-Mains not displayed on Previous Year question paper");
			WaitStatementLib.explicitWaitForVisiblity(driver, 10, samplePprLst_NeetCatalystUser.get(0));
			Assert.assertTrue(samplePprLst_NeetCatalystUser.size() > 5, "All Sample papers not displayed");
			allBtnssamplePprLst_NeetCatalystUser.get(0).click();
			BasePage.sleepForMilliSecond(1000);
	...
	public void ncertregistration() {
		...
		createbtn.click();
		/*
		 * nametxt.sendKeys("werty"); emailtxt.sendKeys("werty@mn.com");
		 * passtxt.sendKeys("123456"); pintxt.sendKeys("110089"); Select board = new
		 * Select(brddrpdwn); board.selectByValue("1"); Select stn =new Select
		 * (classselect); stn.selectByValue("10"); usertype.click(); joinfree.click();
		 */
	}
	...
	public void verifyDashboard(WebDriver driver) {
		...
		BasePage.scrollDown(whatNextText, driver);
		String expectedText = "What Next?";
		Assert.assertEquals(expectedText, whatNextText.getText());

		Assert.assertTrue(whatNextContent.size()> 0);
        for (int i = 0; i <= whatNextContent.size() - 1; i++) {
        Assert.assertTrue(BasePage.isPresentAndDisplayed(whatNextContent.get(i)));
		}
        
        String expectedText2 = "Trending Videos";
        Assert.assertEquals(expectedText2, "Trending Videos");
        
        Assert.assertTrue(trendingVideosContent.size()>0);
        for(int i=0; i<= trendingVideosContent.size() -1; i++) {
        Assert.assertTrue(BasePage.isPresentAndDisplayed(trendingVideosContent.get(i)));	
        }
